...
| **IA** | API OpenAI (ou compatible) | Génération de contenus, analyse, corrections |
...
| **Sync Google Calendar** | Dernière version en cache affichée. Synchronisation automatique au retour de la connexion. |
...
| Contexte | Fonctionnalité IA |
|---|---|
| **Programme** | Parser un PDF du BO → extraction structure (phase 2) |
| **Séquences** | Proposer un découpage en séances |
| **Séances** | Générer déroulé, activités, trace écrite |
| **Cahier de textes** | Synthétiser depuis la séance |
| **Bibliothèque** | Classifier et router les documents importés |
| **Corrections** | Analyser copie → niveaux compétences + réussites/lacunes |
| **Bilan devoir** | Commentaire synthétique classe |
| **Bulletins** | Appréciations discipline et PP |
| **Orientation** | Synthèse forces, axes de progrès, recommandations |
| **Contenus** | Cours, fiches, sujets, corrigés, diaporamas |
...
## 10. État d'implémentation (v1.3 — 4 mars 2026)

### 10.1 Vue d'ensemble

| Métrique | Valeur |
|---|---|
| **Fichiers source** | 182 fichiers (99 TS/TSX, 45 CSS, 3 SQL, configs) |
| **Lignes TypeScript/TSX** | ~15 000 |
| **Lignes CSS** | ~6 000 |
| **Lignes SQL** | ~2 000 |
| **Services backend** | 14 services |
| **Pages/écrans** | 17 pages |
| **Composants UI** | 15 composants réutilisables |
| **Formulaires** | 9 modals/formulaires |
| **Seeds SQL** | 3 fichiers (schéma + prompts IA + programmes) |
...
| `aiService` | `aiService.ts` | 456 | Moteur IA 3 couches, `assemblePrompt`, `smartGenerate`, `smartCorrect`, queue |
...
3. **User instructions** : consignes libres par requête (champ textarea dépliable)
...
#### Emploi du temps (`EmploiDuTempsPage.tsx` — 210 lignes)
...
- Données depuis `timetableService` avec fallback mock
...
#### Générateur IA (`GenerateurIAPage.tsx` — 601 lignes)
...
- Textarea d'instructions utilisateur (couche 3)
...
- **IA & templates** : éditeur de prompts par tâche, mode simple/avancé, `{{variables}}`, picker
  - **Clé API** : champ password + toggle visibilité + sauvegarde keyring OS
